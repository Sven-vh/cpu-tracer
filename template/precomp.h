...
#define GRIDDIMENSIONS	(WORLDSIZE/BRICKSIZE)
#define VOXELSIZE	(1.0f/WORLDSIZE)
#else
#define GRIDSIZE	WORLDSIZE
#endif
#define GRIDSIZE2	(GRIDDIMENSIONS*GRIDDIMENSIONS)
#define GRIDSIZE3	(GRIDDIMENSIONS*GRIDDIMENSIONS*GRIDDIMENSIONS)


//#define SCRWIDTH	(1024)
//#define SCRHEIGHT	(542)
#define SCRWIDTH	(1024)
#define SCRHEIGHT	(542)
#define SCREEN_SIZE (SCRWIDTH * SCRHEIGHT)
...
inline bool bilinearTextures = true;
...
struct Line {
	...
	Line(const float3& start, const float3& end, const float4& color, const float duration = 0.0f) : start(start), end(end), color(color), duration(duration) {}
