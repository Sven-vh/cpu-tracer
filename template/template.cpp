...
//Source: https://github.com/jpaver/opengametools/blob/master/src/ogt_vox.h
...
void ReshapeWindowCallback(GLFWwindow*, int w, int h) {
	renderer->camera.aspect = (float)w / (float)h;
	renderer->camera.UpdateProjection();
	glViewport(0, 0, w, h);
}
void QuitTemplate() { running = false; }
void KeyEventCallback(GLFWwindow*, int key, int, int action, int) {
	if (action == GLFW_PRESS) { if (renderer) if (key >= 0) renderer->KeyDown(key); keystate[key & 255] = 1; } else if (action == GLFW_RELEASE) { if (renderer) if (key >= 0) renderer->KeyUp(key); keystate[key & 255] = 0; }
}
...
void MouseButtonCallback(GLFWwindow*, int button, int action, int) {
	if (action == GLFW_PRESS) { if (renderer) renderer->MouseDown(button); } else if (action == GLFW_RELEASE) { if (renderer) renderer->MouseUp(button); }
}
void MouseScrollCallback(GLFWwindow*, double, double y) {
	renderer->MouseWheel((float)y);
}
void MousePosCallback(GLFWwindow* activeWindow, double x, double y) {
	float xscale, yscale;
	int width, height;
	glfwGetWindowSize(activeWindow, &width, &height);
	xscale = (float)scrwidth / (float)width;
	yscale = (float)scrheight / (float)height;
	x *= xscale, y *= yscale;
	if (renderer) renderer->MouseMove((int)x, (int)y);
}
...
void main() {
	...
	if (strstr(rendererChar, "Arc")) IGP_detected = false;

	//load shader file

	std::string shaderString = TextFileRead("assets/shader.glsl");

	//create shader
	shader = new Shader(
		"#version 330\nin vec4 p;\nin vec2 t;out vec2 u;void main(){u=t;gl_Position=p;}",
		shaderString.c_str(), true);
	...
	if (f) {
		...
		uint base_size = (uint)(((uchar*)&renderer->end_of_base_class + 4) - (uchar*)renderer);
		...
		if (version == 0x10001006 && appSize == rendererSize) {
			...
			void* target = ((uchar*)renderer + sizeof(Renderer)) - rendererSize;
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (frameNr++ > 1) {
			...
			if (renderer->screen) renderTarget->CopyFrom(renderer->screen);
			shader->Bind();
			...
			ImGui::NewFrame();
			renderer->uiUpdated = true;
			renderer->UI(); // app->uiUpdated will be false if Render::UI() was not implemented
			if (renderer->uiUpdated) {
				ImGui::Render();
